...
namespace myApp
{
    class nativeAPIplaystocksTask4
    {
			...
			try {
				...
				while(reader.Read()){
					...
					result = 	(string) reader[reader.GetOrdinal("Name")] + 
								...
								(decimal) reader[reader.GetOrdinal("High")] + 
								(decimal) reader[reader.GetOrdinal("Low")] + 
								(decimal) reader[reader.GetOrdinal("StockOpen")] +  
								(decimal) reader[reader.GetOrdinal("StockClose")] + 
